...
extension TrackerRecordStore: TrackerRecordStoreProtocol {
    ...
    func addRecord(_ record: TrackerRecord) throws {
        ...
        request.predicate = NSPredicate(format: "%K == %@", #keyPath(TrackerCoreData.trackerId), record.trackerId as NSUUID)
    ...
    func deleteRecord(_ record: TrackerRecord) throws {
        let request = NSFetchRequest<TrackerRecordCoreData>(entityName: "TrackerRecordCoreData")
        request.predicate = NSPredicate(format: "%K == %@ AND %K == %@",
                                        #keyPath(TrackerRecordCoreData.trackerId), record.trackerId as NSUUID,
                                        #keyPath(TrackerRecordCoreData.date), record.date as NSDate)
